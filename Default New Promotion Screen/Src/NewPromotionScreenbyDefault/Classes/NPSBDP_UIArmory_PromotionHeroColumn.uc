class NPSBDP_UIArmory_PromotionHeroColumn extends UIPanel;
...
function NPSBDP_UIArmory_PromotionHeroColumn InitPromotionHeroColumn(int InitRank, int InitOffset)
{
...
function PreviewAbility(int idx)
{
	...
	`LOG("PreviewAbility" @ Offset @ idx,, 'PromotionScreen');
	PromotionScreen.PreviewAbility(Rank, Offset + idx);
}
...
function SelectAbility(int idx)
{
	...
	else if (bEligibleForPurchase && PromotionScreen.CanPurchaseAbility(Rank, Offset + idx, AbilityNames[idx]))
	{
		`LOG("ConfirmAbilitySelection" @ AbilityNames[idx] @ Offset @ idx,, 'PromotionScreen');
		PromotionScreen.ConfirmAbilitySelection(Rank, Offset + idx);
	}
	...
	foreach AbilityIcons(AbilityIcon, idx)
	{
		if (Panel == AbilityIcon)
		{
			...
			else if (cmd == class'UIUtilities_Input'.const.FXS_L_MOUSE_IN)
			{
				...
				if( !NPSBDP_UIArmory_PromotionHero(Screen).IsAbilityLocked(Rank) )
				{
